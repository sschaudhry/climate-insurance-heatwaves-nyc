...
# compute tiered payouts per event based on TriggerHI
df3['PayoutPerEvent'] = np.where(df3['TriggerHI'] < 92, 25,
                                 np.where(df3['TriggerHI'] < 95, 50, 75))
# cap total annual payout at $300 per household
df3['TotalPayout'] = np.minimum(df3['Events'] * df3['PayoutPerEvent'], 300)
fig = plt.figure(figsize=(10,6))
# Expanded grid with extra spacing for subtitle/tier
gs = fig.add_gridspec(nrows=6, ncols=1, height_ratios=[0.6,4,0.5,0.5,1,0.7], hspace=1.2)
...
### Subtitle & Tier Explanation Row (span two small rows)
ax_sub = fig.add_subplot(gs[2:4,0])
ax_sub.axis('off')
subtitle = 'Lower trigger thresholds concentrate payouts in vulnerable neighborhoods'
tierinfo = 'Tiered  payouts:  $25  for  88–92°F,  $50  for  92–95°F,  $75  for  ≥95°F.  Annual  cap  =  $300  per  household.'
ax_sub.text(0.5,0.75,subtitle,fontsize=13,style='italic',ha='center',va='center',fontfamily='Arial')
ax_sub.text(0.5,0.35,tierinfo,fontsize=10,ha='center',va='center',fontfamily='Arial')

### Subtitle Row
    ### Table Row follows in next grid row

### Table Row
ax_table = fig.add_subplot(gs[4,0]); ax_table.axis('off')
 # Prepare table data with new payout columns
table_data = df3[['Neighborhood','PctWithoutAC','MedianIncome','PayoutPerEvent','TotalPayout']].values
col_labels = ['Neighborhood','% Without AC','Median Income','Payout/Event\n($)','Total Annual\nPayout ($)']
tbl = ax_table.table(cellText=table_data, colLabels=col_labels, loc='center')
tbl.auto_set_font_size(False)
tbl.set_fontsize(10)
# scale cells (width, height)
tbl.scale(1.2, 1.4)
# set column widths
tbl.auto_set_column_width(col=list(range(len(col_labels))))
col_widths = [0.25, 0.2, 0.2, 0.2, 0.2]
...
for (i, j), cell in tbl.get_celld().items():
    ...
    # Style header row with bold text and grey background
    if i == 0:
        # Smaller font for payout columns to prevent overlap
        if j in [3, 4]:
            cell.set_text_props(weight='bold', fontsize=8, ha='center', va='center')
        else:
            cell.set_text_props(weight='bold', fontsize=11, ha='center', va='center')
        cell.PAD = 0.05
        cell.set_facecolor('#E6E6E6')
    # Reduce font size for data cells in payout columns
    elif j in [3, 4]:
...
plt.savefig('trigger_vs_events.png',bbox_inches='tight')
plt.close()
